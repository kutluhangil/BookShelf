...
import type { AuthSetup } from "./auth";
...
/** Requests per window, per authenticated user (or per address when auth is off). */
const AI_RATE_LIMIT = { windowMs: 60_000, max: 20 };

export interface AppOptions {
  /** The Gemini client. Injected so a test can exercise the routes without one. */
 ...
 */
function describeError(error: unknown, isProduction: boolean): { message: string; detail?: string } {
  if (!(error instanceof Error)) return { message: String(error) };
  if (isProduction) return { message: error.message };
