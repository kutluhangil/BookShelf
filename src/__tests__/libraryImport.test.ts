...
import { parseCsv, parseCsvRows, parseLibraryCsv, rowsToBooks } from '../services/libraryImport';
...
describe('parseCsv', () => {
  ...
  it('tags each row with the line it started on', () => {
    const rows = parseCsvRows(['a,b', '', '"one', 'two",x', 'y,z'].join('\n'));
    expect(rows.map((row) => row.line)).toEqual([1, 3, 5]);
  });
});

describe('parseLibraryCsv', () => {
  it('reports the source line of a skipped row, not its position after filtering', () => {
    const csv = [
      'Title,Author,My Review', // line 1
      'Dune,Herbert,"first', // line 2
      'second"', // line 3
      '', // line 4
      ',Nobody,', // line 5: no title, skipped
    ].join('\n');

    const result = parseLibraryCsv(csv);

    expect(result.rows).toHaveLength(1);
    expect(result.skipped).toEqual([{ line: 5, reason: 'missing-title' }]);
  });
